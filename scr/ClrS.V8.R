...
# input folders
polDir <- file.path(BaseDir, "parsedMrk")
cnvDir <- file.path(BaseDir, "CNV")

# output folders
resDir <- file.path(BaseDir, "Introgressions")
...
myMarkers <- paste0(BaseDir,"/rep/Type1TableMrk")
...
if (ref1Label == "Scc") {
  ...
  purgeOut <- read.delim(paste0(BaseDir,"/scr/Excl_markers_positions.txt"))
  purgeOut$chr <- paste0(purgeOut$chr,"_Scc_",purgeOut$pos)
  dfGG <- as.data.frame(cbind(dfGG,paste0(dfGG[,1],"_",dfGG[,2])))
  colnames(dfGG)[7] <- "ExMark"
  
  dfGG <- dfGG[-c(which(is.element(dfGG$ExMark,purgeOut$chr) == TRUE)),]
  dfGG <- dfGG[,-c(ncol(dfGG))]
}
...
for (ind1 in sampIDs) {
  ...
  ploidy <- as.numeric(tabinfostrains[tabinfostrains[,1] == ind1,2])
  
  if (TRUE) {
    
    cat("Unkown ploidy or ploidy > 2")
